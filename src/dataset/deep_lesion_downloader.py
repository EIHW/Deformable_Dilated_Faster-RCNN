...
link_csv = 'https://raw.githubusercontent.com/anir16293/Deep-Lesion/master/DL_info.csv'


def download_zips(dataset_dir):
    """
    Download the zip files containing the complete DeepLesion dataset.
    :param dataset_dir: The directory to save the dataset to.
    :return the path to the csv file and folder containing the zip-folders.
    """
    csv_file = os.path.join(dataset_dir, 'DL_info.csv')
    data_dir = os.path.join(dataset_dir, 'Images_png')
    os.makedirs(data_dir, exist_ok=True)
    req.urlretrieve(link_csv, csv_file)
    zip_links = {'Images_png_{:02d}.zip'.format(idx+1): links[idx] for idx in range(len(links))}
    for file_name, link in tqdm(zip_links.items(), desc='Download'):
        file_path = os.path.join(data_dir, file_name)
        if not os.path.exists(file_path):
            req.urlretrieve(link, file_path)
    return csv_file, data_dir
